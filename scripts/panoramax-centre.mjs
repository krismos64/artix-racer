// Placage photographique HAUTE FIDÉLITÉ du corridor commerçant du
// centre-bourg : Leclerc Express et sa station, Au Comptoir, le groupe
// scolaire Jean Moulin, la mairie, les commerces de la République et l'église
// Saint-Pierre.
//
// Différences avec le placage général (panoramax-facades-photo.mjs) :
// - le corridor est défini par des ANCRES réelles (positions des POI) ;
// - TOUTES les arêtes bien photographiées d'un bâtiment sont plaquées, pas
//   seulement la meilleure : un commerce d'angle garde ses deux devantures ;
// - la rectification part des panoramiques HD (5760×2880) : les enseignes,
//   logos et inscriptions deviennent lisibles ;
// - cases de 1024×512 et fondu de pied réduit à 60 cm, sous les vitrines.
//
// Sortie : public/textures/facades-centre-*.jpg + artix-facades-centre.json.
// Licence : Panoramax IGN, Licence Ouverte 2.0.
import { readFileSync, writeFileSync, mkdirSync, existsSync } from 'node:fs';
import sharp from 'sharp';

const ORIGIN = { lat: 43.39743, lon: -0.57224 };
const R = 6378137;
const CACHE = '.panoramax-cache-hd';
// Ancres du corridor (coordonnées locales du jeu, relevées dans les POI) et
// rayon de couverture autour de chacune.
const ANCRES = [
  [99, -36],    // E. Leclerc Express et sa station
  [45, -21],    // Au Comptoir
  [-52, -31],   // École élémentaire Jean Moulin
  [-13, -126],  // École maternelle Jean Moulin
  [-52, -110],  // Collège Jean Moulin
  [-12, 50],    // Mairie
  [8, 107],     // Pharmacie de la République et commerces
  [12, 170],    // Église Saint-Pierre
];
const RAYON_ANCRE = 85;
const MAX_FACADES = 224;
const CAM_H = 2.4;             // hauteur de la GoPro au-dessus de la chaussée
const CASE_W = 1024, CASE_H = 512;
const ATLAS_GRILLE = 4;        // 4×8 cases de 1024×512 dans un atlas 4096×4096
...
for (const b of batiments) {
  ...
  const dCentre = Math.min(...ANCRES.map(([ax, az]) => Math.hypot(b.cx - ax, b.cz - az)));
  if (dCentre > RAYON_ANCRE) continue;
...
async function telecharger(id) {
  ...
  const url = `https://panoramax.ign.fr/api/pictures/${id}/hd.jpg`;
...
function rectifier(pano, W, H, photo, A, B, hMur) {
  ...
  // 60 cm seulement : le corridor est commerçant, un fondu plus haut
  // effaçait le bas des vitrines. Quelques roues photographiées peuvent
  // rester en pied de mur, moins gênantes qu'une devanture amputée.
  const yFondu = Math.min(CASE_H - 1, Math.round((1 - 0.6 / hMur) * CASE_H));
  const ref = [0, 0, 0];
  let nRef = 0;
...
for (let na = 0; na < atlasUtiles; na++) {
  ...
  }).jpeg({ quality: 82 }).toFile(`public/textures/facades-centre-${na}.jpg`);
}
writeFileSync('public/data/artix-facades-centre.json', JSON.stringify({
  source: 'Panoramax IGN, Licence Ouverte 2.0 — façades HD du corridor du centre-bourg',
...
console.log('Écrit : public/textures/facades-centre-*.jpg + public/data/artix-facades-centre.json');
