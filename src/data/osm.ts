...
export function findSpawn(roads: any[], buildings: any[] = []) {
  ...
  for (const r of roads) {
    ...
    for (let i = 0; i < r.pts.length - 1; i++) {
      ...
      // Le départ doit être immédiatement identifiable : on privilégie les
      // voies nommées du centre, autour de la mairie, plutôt qu'une voie de
      // service anonyme qui serait parfois quelques mètres plus proche.
      const mairieDistance = Math.hypot(mx + 4, mz - 70);
      const score = mairieDistance - r.width * 2.5 - Math.min(segLen, 45)
        - (r.name ? 70 : 0);
      if (score < bestScore) {
        bestScore = score;
        best = {
