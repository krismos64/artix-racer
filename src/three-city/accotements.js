...
const REVETEMENTS = {
  ...
  trottoir:   { couleur: 0x716f68, largeur: 1.6, creux: 0.00, exhaussement: 0.14 },
...
const BORDURE_COULEUR = 0x96938a;
...
export class Accotements {
  constructor(data, relief, roadY) {
    ...
    for (const [cle, pos] of parRev) {
      ...
      // Variation par dalle de 4 m. Une teinte parfaitement uniforme faisait
      // lire le trottoir comme une grande surface blanche sans matière.
      if (cle === 'trottoir') {
        const base = new THREE.Color(rev.couleur);
        const couleurs = [];
        for (let i = 0; i < pos.length; i += 3) {
          const x = pos[i], z = pos[i + 2];
          const graine = Math.sin((Math.floor(x / 4) * 17.1 + Math.floor(z / 4) * 31.7)) * 43758.5453;
          const variation = 0.84 + (graine - Math.floor(graine)) * 0.22;
          couleurs.push(base.r * variation, base.g * variation, base.b * variation);
        }
      ...
      const m = new THREE.Mesh(g, new THREE.MeshStandardMaterial({
        color: cle === 'trottoir' ? 0xffffff : rev.couleur,
        vertexColors: cle === 'trottoir', roughness: cle === 'trottoir' ? 0.94 : 0.96,
